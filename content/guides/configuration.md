...
| `control_footnotes` | `[extra.policypress]` | bool | `false` | Accept native Markdown footnote references to controls (`[^IAC-01]`) in policy bodies, as an alternative to the `{{/* control(id="IAC-01") */}}` shortcode (see [Inline control references](@/guides/compliance-frameworks.md#inline-control-references)). When `true`, the site build must run `policypress stage-site` (the GitHub Action does automatically) so the website gets synthesised footnote definitions the way the PDF already does; a bare reference to a *known* SCF id then stops being a `--strict` error, while an *unknown* id stays one. The shortcode keeps working regardless. |
